...
**[推论] Hermes 本地映射：** 本页定义上下文与检索信任边界；[[production-ai-agent-evaluation-framework]] 负责将语料选择、召回、租户隔离、引用覆盖和主张支撑拆分验证；[[agent-development-lifecycle]] 负责把失败案例送回评测与迭代；[[agent-context-engineering]] 负责更宽的上下文装配与工具选择边界。
